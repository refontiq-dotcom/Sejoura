-- Phase 10 regression fix: rooms is scoped through its accommodation and has no tenant_id column.
...
BEGIN
  SELECT accommodation_id INTO v_room_accommodation
  FROM public.rooms WHERE id = NEW.room_id;

  SELECT tenant_id INTO v_accommodation_tenant
  FROM public.accommodations WHERE id = NEW.accommodation_id;

  SELECT tenant_id INTO v_client_tenant
  FROM public.clients WHERE id = NEW.client_id;

  IF v_room_accommodation IS NULL OR v_accommodation_tenant IS NULL OR v_client_tenant IS NULL THEN
