-- ============================================================================
-- Migration : Calcul automatique de la taxe de nuitée à la réservation
-- Date      : 2026-09-19
-- ============================================================================
-- Contexte : suite de la migration 20260919_tourist_tax.sql (colonnes de
-- configuration). Ici, create_booking() et update_booking() calculent
-- automatiquement bookings.tourist_tax_amount à partir de la configuration
-- de l'établissement (accommodations.tourist_tax_enabled / tourist_tax_rate),
-- du nombre de nuits et du nombre d'occupants — sans changer la signature
-- des fonctions, donc sans aucune modification requise côté application.
--
-- Formule : tourist_tax_amount = tourist_tax_rate × nights_count × guests
-- Si la taxe n'est pas activée sur l'établissement, le montant reste à 0.
-- ============================================================================

-- ── 1. create_booking : calcule le montant à la création ───────────────────
...
BEGIN
  ...
  -- Taxe de nuitée : calculée uniquement si activée sur l'établissement
  SELECT CASE WHEN a.tourist_tax_enabled AND a.tourist_tax_rate IS NOT NULL
              THEN a.tourist_tax_rate * p_nights_count * GREATEST(p_number_of_guests, 1)
              ELSE 0
         END
  ...
  INSERT INTO bookings (
    ...
    base_price, negotiated_price, nights_count, total_amount, tourist_tax_amount,
    ...
    p_base_price, p_negotiated_price, p_nights_count, p_total_amount, COALESCE(v_tourist_tax_amount, 0),
...
-- ── 2. update_booking : recalcule si les nuits changent ─────────────────────
...
BEGIN
  ...
  -- Taxe de nuitée : recalculée si le nombre de nuits change (établissement
  -- inchangé lors d'une modification de réservation, donc son tarif suffit).
  SELECT CASE WHEN a.tourist_tax_enabled AND a.tourist_tax_rate IS NOT NULL
              THEN a.tourist_tax_rate * v_nights * GREATEST(v_booking.number_of_guests, 1)
              ELSE 0
         END
  INTO v_tourist_tax_amount
  FROM accommodations a
  WHERE a.id = v_booking.accommodation_id;
  ...
  SET check_in_date      = v_new_check_in,
      ...
      tourist_tax_amount = COALESCE(v_tourist_tax_amount, 0),
      payment_status     = CASE
