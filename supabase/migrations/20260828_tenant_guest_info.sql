-- 20260828_tenant_guest_info.sql
-- Conditions & infos client configurables par l'établissement (Paramètres →
-- Espace client) et affichées intelligemment sur la page de l'espace client
-- (/stay) : infos pratiques, règlement intérieur, note de check-in et numéro
-- d'urgence.
--
-- Stockage : colonne JSONB `guest_info` sur `tenants` (globale à l'établissement,
-- conforme au choix de l'admin : pas de saisie par résidence).
--
--   {
--     "practical_info": [ { "icon": "wifi", "label": "Wi-Fi", "value": "Séjoura-5G" } ],
--     "house_rules":    [ { "title": "Fumer est interdit", "description": "..." } ],
--     "checkin_note":   "Arrivée après 20h : prévenir la réception",
--     "emergency_phone": "+225 07 00 00 00 00"
--   }
--
-- Le tout est idempotent : re-exécutable sans erreur.

-- ----------------------------------------------------------------------------
-- 1. COLONNE: tenants.guest_info
-- ----------------------------------------------------------------------------
ALTER TABLE tenants
  ADD COLUMN IF NOT EXISTS guest_info JSONB NOT NULL DEFAULT '{}'::jsonb;

-- ----------------------------------------------------------------------------
-- 2. FONCTION: get_client_stay(p_token)
--    Étendue pour retourner guest_info afin que la page /stay puisse afficher
--    les infos pratiques, le règlement et le numéro d'urgence configurés.
--    (Définition complète reprise, seule la partie tenant change.)
-- ----------------------------------------------------------------------------
