...
DROP FUNCTION IF EXISTS check_in_booking CASCADE;
DROP FUNCTION IF EXISTS update_booking CASCADE;
DROP FUNCTION IF EXISTS detect_overdue_checkouts CASCADE;
...
CREATE TABLE bookings (
  ...
  is_overdue      BOOLEAN NOT NULL DEFAULT FALSE,  -- Départ prévu dépassé (client encore arrivé)
  overdue_since   TIMESTAMPTZ,                     -- Depuis quand le départ est dépassé
...
CREATE INDEX idx_bookings_overdue ON bookings(status, is_overdue)
...
CREATE TABLE cleaning_tasks (
  ...
  alert_time      TIMESTAMPTZ,                  -- checkout_time + 1h30 (ou prochaine arrivée)
  force_release_time TIMESTAMPTZ,               -- checkout_time + 2h (ou prochaine arrivée)
  next_arrival_at TIMESTAMPTZ,                  -- Prochaine arrivée prévue sur la chambre
...
-- ----------------------------------------------------------------------------
-- 19b. TRIGGER: Réinitialisation du signalement de départ dépassé au check-out
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 27b. FONCTION: Modifier / prolonger une réservation
...
-- ----------------------------------------------------------------------------
-- 27c. FONCTION: Détecter les départs dépassés
...
-- ----------------------------------------------------------------------------
-- 28. FONCTION: Check-in d'un client
--     Garde-fous : interdit le check-in avant la date d'arrivée prévue
--     (arrivée anticipée) et après la fin du séjour (trop tard), sauf si
--     p_allow_early / p_allow_late sont passés à TRUE explicitement.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 34d. NOTIFICATIONS SYSTÈME — génération automatique de notifications à
