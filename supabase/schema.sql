...
CREATE TABLE bookings (
  ...
  is_overstay     BOOLEAN NOT NULL DEFAULT FALSE,      -- Dépassement de séjour détecté
  overstay_detected_at TIMESTAMPTZ,                     -- Premier signalement du dépassement
  overstay_auto_checked_out BOOLEAN NOT NULL DEFAULT FALSE, -- Auto check-out après délai de grâce
...
CREATE INDEX idx_bookings_overstay ON bookings(tenant_id)
...
-- ----------------------------------------------------------------------------
-- 35b. FONCTION: check_overstays()
--    Détecte les séjours dépassés (client encore en chambre après le départ
--    prévu), envoie une alerte au personnel puis auto check-out après un délai
--    de grâce. À appeler périodiquement (pg_cron ou frontend).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 35c. FONCTION: extend_booking()
--    Prolonge le séjour d'une réservation active (confirmée ou arrivée) :
--    nouvelle date de départ, recalcule des nuits et du montant total dû,
